 ...
 * @since 1.0.2
 */
public class DispatcherHttpServletResponse implements HttpServletResponse {
	 ...
	 * <p>
	 * If {@code false}, {@link #getCharacterEncoding()} will return a default encoding
	 * value.
	 */
	 ...
	 * using the charset specified for the response by the application, either through
	 * {@link HttpServletResponse} methods or through a charset parameter on the
	 * {@code Content-Type}.
	 * @return the content as a {@code String}
	 * @throws UnsupportedEncodingException if the character encoding is not supported
	...
	private String getCookieHeader(Cookie cookie) {
		...
		if (maxAge >= 0) {
			...
			buf.append("; Expires=");
			ZonedDateTime expires = (cookie instanceof DispatcherCookie ? ((DispatcherCookie) cookie).getExpires() : null);
			if (expires != null) {
		...
		if (cookie instanceof DispatcherCookie) {
			DispatcherCookie mockCookie = (DispatcherCookie) cookie;
	 ...
	 * <p>
	 * As of Servlet 3.0, this method is also defined in HttpServletResponse. As of Spring
	 * 3.1, it returns a stringified value for Servlet 3.0 compatibility. Consider using
	 ...
	 * <p>
	 * As of Servlet 3.0, this method is also defined in HttpServletResponse. As of Spring
	 * 3.1, it returns a List of stringified values for Servlet 3.0 compatibility.
	...
	private boolean setSpecialHeader(String name, Object value, boolean replaceHeader) {
		if (HttpHeaders.CONTENT_TYPE.equalsIgnoreCase(name)) {
			...
			DispatcherCookie cookie = DispatcherCookie.parse(value.toString());
			if (replaceHeader) {
				setCookie(cookie);
			}
			else {
	...
	/**
	 * Set the {@code Set-Cookie} header to the supplied {@link Cookie}, overwriting any
	 * previous cookies.
	...
	/**
	 * Inner class that adapts the PrintWriter to mark the response as committed once the
